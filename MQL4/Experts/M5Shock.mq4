...
#define Magic 346413245

#include <M5ShockParameter.mqh>
...
//--------------------------------------------------------------------------   
   //运行时开关变量
   bool canOpenOrder=IsInTradeTime() && TimeDistance(LastOpenOrderTime,MainTimeFrame*4*60);
...
int GetSignal(int& Signal[])
{
   ...
   //平空
   if(OpenOrderCount>0 && M5Shock(Symbol(),20)>0) Signal[7]=10;
   //平多
   if(OpenOrderCount>0 && M5Shock(Symbol(),20)<0) Signal[6]=10;
      
   //做空
   if(OpenOrderCount==0 && M5Shock(Symbol(),20)<0)
   {
      MainDirection=-1;
      Signal[1]=10;
      ...
      if(OrderSelect(i,SELECT_BY_POS))
      {
         if(OrderMagicNumber()==magic && OrderSymbol()==symbol)
         {
            if(OrderType()<2)//OP_SELL or OP_BUY
            {
               RefreshRates();
               double v;
               if(OrderType()==OP_SELL) v=Ask;
               else v=Bid;
               if(TimeDistance(OrderOpenTime(),5400)) OrderClose(OrderTicket(),OrderLots(),NormalizeDouble(v,Digits),Slip,BuyColor);
            }
